...
| **AgentBuilder**         | `src/agentBuilder/AgentBuilder.ts`    | Indexes `dataSources` content and agent directories; creates/loads platform-specific agent files for GitHub (`.agent.md`), Claude (`.md` in `.claude/agents`), and Codex (`AGENTS.md`) with JSON companions, including Codex multi-entry collections (`agents[]`) and directory targeting via `codexAgentPaths` |
...
11. **Multi-platform agent generation**: AgentBuilder can generate and round-trip agent definitions for GitHub, Claude, and Codex (including `AGENTS.md`) with platform-specific path inference and companion JSON metadata.
